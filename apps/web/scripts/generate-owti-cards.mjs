// Generates a downloadable "wellness type" card image (1080×1350) for every one
// of the 16 OWTI types, into apps/web/public/owti/cards/owti-<code>.<ext>.
//
...
//
// Run from the repo root (playwright-core is a devDependency of apps/web):
//   pnpm --filter web cards:generate
//
// Output format defaults to JPEG, which keeps all 16 committed cards to a couple
// of MB and is what every social app accepts on upload. Override per run:
//   CARD_FORMAT=png  pnpm --filter web cards:generate   # lossless, ~1.2MB each
//   CARD_QUALITY=0.8 pnpm --filter web cards:generate
//
// Chromium resolution order: $CHROMIUM_PATH → playwright-core's configured
// browser. playwright-core does NOT download browsers, so point $CHROMIUM_PATH
// at an installed Chrome/Edge, or install the full `playwright` package once.
...
// This script lives in apps/web/scripts, so `..` is the web app and `../../..`
// is the monorepo root. The OWTI domain moved to packages/shared when the repo
// became a pnpm workspace, so its types are resolved from there, not from the
// app's `src/application/owti` barrel (which only re-exports the package).
const APP_ROOT = join(dirname(fileURLToPath(import.meta.url)), "..");
const REPO_ROOT = join(APP_ROOT, "..", "..");
const TYPES_TS = join(REPO_ROOT, "packages/shared/src/owti/types.ts");
const EMOJI_TS = join(APP_ROOT, "src/app/api/og/emojiSvg.ts");
const FONT_FILE = join(APP_ROOT, "src/app/fonts/PretendardVariable.woff2");
const OUT_DIR = join(APP_ROOT, "public/owti/cards");

// Encoding for the written files. JPEG at 0.92 is visually indistinguishable
// from the PNG on this flat, text-heavy artwork at roughly a tenth the size.
const FORMATS = { jpeg: "jpg", webp: "webp", png: "png" };
const FORMAT = (process.env.CARD_FORMAT ?? "jpeg").toLowerCase();
const QUALITY = Number(process.env.CARD_QUALITY ?? "0.92");
if (!(FORMAT in FORMATS)) {
  throw new Error(
    `CARD_FORMAT must be one of ${Object.keys(FORMATS).join(" | ")}, got "${FORMAT}"`,
  );
}
if (!(QUALITY > 0 && QUALITY <= 1)) {
  throw new Error(`CARD_QUALITY must be in (0, 1], got "${QUALITY}"`);
}
...
window.__renderCard = async (code, mime, quality) => {
  ...
  // A codec Chromium cannot encode silently falls back to PNG — fail loudly
  // rather than writing a PNG under a .jpg/.webp name.
  if(!url.startsWith("data:"+mime+";base64,")){
    throw new Error("canvas could not encode "+mime);
  }
  return url;
};
</script></body></html>`;
...
async function main() {
  ...
  for (const t of types) {
    const dataUrl = await page.evaluate(
      ([code, mime, quality]) => window.__renderCard(code, mime, quality),
      [t.code, `image/${FORMAT}`, QUALITY],
    );
    const bytes = Buffer.from(dataUrl.split(",")[1], "base64");
    const file = join(OUT_DIR, `owti-${t.code.toLowerCase()}.${EXT}`);
    await writeFile(file, bytes);
    total += bytes.length;
    console.error(`  ✓ ${t.code}  ${t.emoji}  ${t.name}  (${bytes.length}B)`);
  }

  await browser.close();
  const mb = (total / 1024 / 1024).toFixed(1);
  console.error(
    `\nwrote ${types.length} ${FORMAT} cards (${mb}MB total) → ${OUT_DIR}`,
  );
}
