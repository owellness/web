      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h2 className="mt-2 text-2xl font-semibold tracking-tight text-foreground">
            해외 웰니스 새 글의 제목과 소개를 한국어로
          </h2>
          <p className="mt-2 max-w-2xl text-sm leading-relaxed text-muted-foreground">
            각 사이트가 공개한 RSS의 제목과 짧은 소개를 번역해 전합니다.
            자세한 내용과 최신 수정 사항은 영문 원문을 확인해 주세요.
          </p>
        </div>
        ...
        <div className="mt-8 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {items.map((item) => (
            <a
              ...
              target="_blank"
              rel="noopener noreferrer external"
              aria-label={`${item.sourceName} 원문에서 ${item.title} 읽기 (새 창)`}
              className="group flex min-h-64 flex-col rounded-2xl border border-border bg-card p-6 transition hover:-translate-y-0.5 hover:border-accent/40 hover:bg-muted/40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent focus-visible:ring-offset-2"
            >
              ...
              <div className="mt-auto flex items-end gap-3 pt-6">
                ...
                <span className="ml-auto text-sm font-medium text-foreground">
                  원문에서 읽기 <span aria-hidden="true">↗</span>
                </span>
              </div>
            </a>
          ))}
